...
%copy first-line%
```bash
$ fluvio topic create github-demo
```
...
We'll be using [this example GitHub SmartModule][2], which we'll learn more about in this section.
If you'd like to create your own SmartModule from scratch, see [the docs on SmartModule Maps][3].

Since we'll be using a pre-written SmartModule, the first thing we need to do is clone the repository:

%copy first-line%
```bash
$ git clone https://github.com/infinyon/fluvio-smartmodule-examples && cd fluvio-smartmodule-examples
```

Then, let's take a look at the GitHub SmartModule example at `github-stars/src/lib.rs`.
...
If you're using a Local Connector, we'll need to stop and restart the connector as well.
