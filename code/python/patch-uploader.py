...
def push_patch(file_patch):
    ...
    # defining what topic to produce to
...
def pull_patch(file_patch):
    ...
    while to_recover >= 1 :
        for record in consumer.stream(Offset.from_end(to_recover)):
            ...
            # call the stream() command and exit the loop as shown here.

            # TODO: experiment with not for-looping stream()

            json_lst.append("{}".format(record.value_string()))
            break
        to_recover -= 1
